...
class CartPoleMorphEnv(gym.Env):
    """
    Description:
        A pole is attached by an un-actuated joint to a cart, which moves along
        a frictionless track. The pendulum starts upright, and the goal is to
        prevent it from falling over by increasing and reducing the cart's
        velocity. The environment also has the added functionality of working with environmental parameters
        as a toy problem for co-adapting robot morphology
    ...
    def __init__(self):
        ...
        # Angle limit set to 2 * theta_threshold_radians so failing observation
        # is still within bounds.
        # environmental parameter bounds added to the beggining.
        high = np.array(
            [
                3.0,
                1.0,
        ...
        self.steps_beyond_done = None
        self.time_step = 0
        self.end_time_step = 500

    # Function to reset the morphology space or dynamic parameters of the experimental environment
    """Allows the user to reset the design parameters of the experiment. Function takes in 
            an array of parameters(similar to how step takes in an action) and resets the environments experimental params"""
    ...
    """Returns the design parameters which have been initialized by the environment or set by the user."""
    ...
    """ As opposed to a regular gym environment we have appended the experimental parameters 
        onto the end of the observation returned by the environment. This is so that researchers who are working with 
        a morphing environment can tell what the design parameters are at any given timestep by splicing the observation
    """
    ...
    def step(self, action):
        ...
        if not done:
            ...
            reward = -100
        self.time_step += 1
        return np.append(np.array([self.masscart, self.masspole, self.length]),
    ...
    """Get and set state functions implemented to interace with the control env with true dynamics. """
    def get_state(self):
       return np.append(np.array([self.masscart, self.masspole, self.length]), np.array(self.state))
    ...
    def PETS_reward_func(self, action):
        ...
        #target_distance = np.abs(target[1] - pole_edge[1])
        reward = - target_distance - 0.01 * np.sum(np.square(action))
        #return reward
        return reward
